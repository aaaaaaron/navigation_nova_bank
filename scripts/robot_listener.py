...
def odom_combined_callback(data):
	...
	q_angle = data.pose.pose.orientation
	qx = q_angle.x
	qy = q_angle.y
	qz = q_angle.z
	qw = q_angle.w
	q = [qx, qy, qz, qw]
	euler = t.euler_from_quaternion(q)
	yaw_bearing = math.degrees(euler[2])
	yaw_bearing = gpsmath.format_bearing(yaw_bearing)

	robot_drive.lon_now = x_lng
